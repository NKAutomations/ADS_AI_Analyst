...
from PySide6.QtWidgets import (
    QApplication, QCheckBox, QDateTimeEdit, QGroupBox, QHBoxLayout, QFormLayout, QDialog, QDialogButtonBox,
    QHeaderView, QLabel, QLineEdit, QMainWindow, QMessageBox, QDoubleSpinBox,
    QPlainTextEdit, QPushButton, QSizePolicy, QSpinBox,
    QSplitter, QStatusBar, QTableWidget, QTableWidgetItem, QVBoxLayout,
    QWidget, QTabWidget, QFileDialog, QScrollArea,
)
...
class SettingsDialog(QDialog):
    ...
    def _build(self):
        ...
        self.ads_timeout = QDoubleSpinBox(); self.ads_timeout.setRange(0.1, 120); self.ads_timeout.setDecimals(1); self.ads_timeout.setSuffix(" s")
        self.cycle_ms = QSpinBox(); self.cycle_ms.setRange(1, 60000); self.cycle_ms.setSuffix(" ms")
        ...
        self.llm_timeout = QDoubleSpinBox(); self.llm_timeout.setRange(1, 3600); self.llm_timeout.setDecimals(1); self.llm_timeout.setSuffix(" s")
        self.context = QSpinBox(); self.context.setRange(256, 1048576); self.context.setSingleStep(256)
        self.temperature = QDoubleSpinBox(); self.temperature.setRange(0, 2); self.temperature.setDecimals(3); self.temperature.setSingleStep(.05)
        self.max_tokens = QSpinBox(); self.max_tokens.setRange(1, 1048576)
        self.top_p = QDoubleSpinBox(); self.top_p.setRange(0, 1); self.top_p.setDecimals(3); self.top_p.setSingleStep(.05)
        self.top_k = QSpinBox(); self.top_k.setRange(0, 100000)
        self.repeat_penalty = QDoubleSpinBox(); self.repeat_penalty.setRange(0, 10); self.repeat_penalty.setDecimals(3); self.repeat_penalty.setSingleStep(.05)
        ...
        self.max_entries = QSpinBox(); self.max_entries.setRange(100, 1000000)
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self._last_values: Dict[str, object] = {}  # fuer Aenderungserkennung
    ...
    def _build_ui(self) -> None:
        ...
        # Rechte Seite: Tabs fuer Analyse / Prompt-Vorschau / Protokoll
        right_widget = QWidget()
        right_layout = QVBoxLayout(right_widget)
        right_layout.addWidget(self._build_analysis_panel())
        right_layout.addWidget(self._build_right_tabs())
    ...
    def _build_variable_panel(self) -> QGroupBox:
        ...
        self.var_table.setHorizontalHeaderLabels([
            "Symbol", "Typ", "TC-Typ", "Kommentar",
            "Anzeigen", "Loggen", "KI", "Wert"
        ])
    ...
    def _build_analysis_panel(self) -> QGroupBox:
        ...
        self.te_prompt.setPlainText(
            self.cfg.get("llm", {}).get(
                "system_prompt",
                "Analysiere ausschliesslich die uebergebenen ADS-Daten und ihren zeitlichen Verlauf. "
                "Beschreibe sachlich Beobachtungen, Auffaelligkeiten und moegliche Ursachen. "
                "Erfinde keine Werte und gib keine Steuerungs- oder Schreibbefehle aus. "
                "Antworte als verstaendlicher Freitext.",
            )
        )
        ...
        self.btn_analyze.setStyleSheet("background-color: #1565c0; color: white; font-weight: bold;")
    ...
    def _build_right_tabs(self) -> QTabWidget:
        """Tabs: Protokoll (nur geloggte Aenderungen) + Prompt-Vorschau"""
        tabs = QTabWidget()
        ...
        info = QLabel("Nur Aenderungen von Variablen mit gesetztem 'Loggen'-Haken werden hier angezeigt.")
        ...
        self.te_log.setPlaceholderText("Hier erscheinen Aenderungen geloggter Variablen...")
        ...
        tabs.addTab(log_widget, "Protokoll (geloggte Aenderungen)")
        ...
        prompt_info = QLabel("Genau das wird bei der naechsten Analyse an die KI gesendet.")
    ...
    def _sync_selection_from_table(self) -> None:
        """Liest alle Checkbox-Zustaende direkt aus der Tabelle und schreibt sie ins state_model."""
        for row in range(self.var_table.rowCount()):
    ...
    def _on_apply_selection(self) -> None:
        ...
        # Zuerst Checkboxen aus der Tabelle ins state_model uebertragen
    ...
    def _on_value_updated(self, symbol: str, value: object, ts: object) -> None:
        ...
        # Nur bei echter Aenderung loggen und ins Protokoll schreiben
    ...
    def _on_analyze(self) -> None:
        ...
        # Prompt-Vorschau automatisch aktualisieren
        self._update_prompt_preview()

        self._append_sys(
            "[LLM] Analyse gestartet | KI-Symbole: " + str(len(ai_vars)) +
            " | Historieneintraege: " + str(len(history))
        )
    ...
    def _append_sys(self, message: str) -> None:
        """Systemmeldungen (Verbindung, Notifications, Fehler) -> Tab 'Systemmeldungen'"""
    ...
    def _open_settings(self) -> None:
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            save_config(self.cfg)
            self.history_model.max_entries = int(self.cfg.get("logging", {}).get("max_entries", 5000))
            self.te_prompt.setPlainText(self.cfg.get("llm", {}).get("system_prompt", ""))
            self.le_llm_url.setText(self.cfg.get("llm", {}).get("base_url", "http://127.0.0.1:1234/v1"))
